...
const { Connection, PublicKey, Keypair, LAMPORTS_PER_SOL, Transaction, SystemProgram, TransactionInstruction } = require('@solana
/web3.js');
const { TOKEN_METADATA_PROGRAM_ID: IMPORTED_TOKEN_METADATA_PROGRAM_ID } = require('@metaplex
-foundation/mpl-token-metadata');
const fsPromises = require('fs').promises;
const fs = require('fs');
const path = require('path');
const splToken = require('@solana
/spl-token');
...
const { TOKEN_PROGRAM_ID: TokenProgramId, createInitializeMintInstruction, getAssociatedTokenAddress, createAssociatedTokenAccountInstruction, createMintToInstruction, ASSOCIATED_TOKEN_PROGRAM_ID } = require('@solana
/spl-token');
const cors = require('cors');
const { exec } = require('child_process');
const { SESClient, SendEmailCommand } = require('@aws
-sdk/client-ses');
const { MongoClient } = require('mongodb');
const { ObjectId } = require('mongodb'); 
const solanaWeb3 = require('@solana
/web3.js');
const { S3Client, HeadObjectCommand, PutObjectCommand } = require('@aws
-sdk/client-s3');
...
const DATA_DIR = path.join(__dirname, 'data');// Define TOKEN_METADATA_PROGRAM_ID with fallback
...
if (!IMPORTED_TOKEN_METADATA_PROGRAM_ID) {
    console.warn('TOKEN_METADATA_PROGRAM_ID not found in @metaplex
-foundation/mpl-token-metadata. Using hardcoded fallback. Consider updating the package.');
}// Initialize Express app
const app = express();
app.use(express.json());
app.use(express.urlencoded({ extended: true })); 
app.use(express.static('public'));
app.use(cors());// Increase request size limit for JSON and form-data
app.use(express.json({ limit: '500mb' }));
app.use(express.urlencoded({ limit: '500mb', extended: true }));app.post('/claim-quest/:username/:type/:questId', async (req, res) => {
...
app.use('/node_modules', express.static('node_modules'))// Ensure these directories exist
...
});const { Metaplex, keypairIdentity, TransactionBuilder } = require('@metaplex
-foundation/js');// Global variables
let users = {};
let posts = [];
let tickets = [];
let blogs = [];
...
}app.use('/uploads', express.static(path.join(__dirname, '..', 'uploads')));let isInitialized = false; // Guard to prevent multiple callsasync function initialize() {
...
}const { LambdaClient, InvokeCommand } = require('@aws
-sdk/client-lambda');const lambdaClient = new LambdaClient({
    region: 'us-east-1',
    credentials: {
        accessKeyId: 'AKIAW5WU5LN7HKW7BNXV',
        secretAccessKey: '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'
...
}initialize().then(() => {});let loggedInUsername = null;async function requireAdmin(req, res, next) {
...
app.post('/unstake/:username/:mintAddress', async (req, res) => {
    ...
    res.sendFile(path.join(__dirname, 'node_modules', '@solana
', 'web3.js', 'dist', 'index.js'));
...
});// [Unchanged imports and setup]// Update /posts/like to return a better message
...
app.post('/posts/delete-comment', async (req, res) => {
    ...
    .map(r => r.route.path));app.get('*', (req, res) => {
