...
}, true);console.log('[Script] Checking for mintNFT overrides...');
...
function stopAllGameTimers() {
    ...
    if (response.ok && data.success) {
        ...
        nfts.forEach(nft => {
            ...
            html += <div class="nft-card ${staked ? 'staked' : ''}">                 <img id="nft-img-${mintAddress}" alt="${stageName}" src="${imageUri}"                      onerror="this.onerror=null; this.src='https://drahmlrfgetmm.cloudfront.net/assetsNFTmain/profilepics/magicseed.png'; console.log('NFT image failed:', '${imageUri}')"                      style="max-width: 200px;">                 <p>${stageName}</p>                 <p>Mint Address: ${mintAddress.slice(0, 8)}...</p>                 <div class="nft-card-buttons">;
            if (showButtons) {
                html += <button class="nft-card-button ${staked ? 'unstake-btn' : 'stake-btn'}"                      onclick="${staked ? 'unstakeNFT' : 'stakeNFT'}('${mintAddress}')">                     ${staked ? 'Unstake' : 'Stake'}                 </button>;
                html += <button class="nft-card-button evolve-btn" onclick="evolveNFT('${mintAddress}', this)"                      ${canEvolve ? '' : 'disabled'}>Evolve</button>;
            }
            html += '</div></div>';
        });
    ...
    if (response.ok && Array.isArray(postsData) && postsData.length > 0) {
        ...
        sortedPosts.forEach((post) => {
            ...
            html +=                  <div class="post-card" data-post-id="${safePostId}">                     <img src="${profilePic}" alt="User Profile Picture" style="width: 30px; height: 30px; border-radius: 50%; vertical-align: middle;">                     <strong>${post.username || 'Anonymous'}</strong> (${date})                     <p>${post.content || 'No content'}</p>;
            if (isAdmin) html += <button onclick="deletePost('${safePostId}')" data-tooltip="Delete Post">Delete</button>;
            html +=                     <p>Likes: <span id="likes-${safePostId}">${post.likes || 0}</span></p>                     <button onclick="likePost('${safePostId}')" ${hasLikedPost ? 'disabled' : ''}>${hasLikedPost ? 'Liked' : 'Like'}</button>                     <button onclick="toggleCommentInput('${safePostId}', 'top')">Comment</button>                     <button onclick="toggleComments('${safePostId}')">Show/Hide Comments (${(post.comments || []).length})</button>                     <div id="comment-input-${safePostId}-top" style="display: none;">                         <textarea id="comment-content-${safePostId}-top" class="comment-input" maxlength="280" placeholder="Add a comment (max 280 chars)"></textarea>                         <button onclick="submitComment('${safePostId}', null)">Submit</button>                     </div>                     <div id="comment-section-${safePostId}" class="comment-section"></div>                 </div>            ;
        });
...
if (response.ok && data.success) {
    ...
    if (response.ok && data.success) {
        ...
        let html =              <img id="profile-pic-display" src="${loggedInProfilePic}" alt="Your Profile Picture" onerror="this.src='https://drahmlrfgetmm.cloudfront.net/assetsNFTmain/profilepics/PFP1.png'; console.log('Profile pic failed:', '${loggedInProfilePic}')">             <p>Username: ${loggedInUsername}</p>             <p>Lemonade Points: ${lemonadePoints.toString()}</p>             <p>Staking Points: ${stakingPoints.toString()}</p>             <p>Arcade Points: ${arcadePoints.toString()}</p>             <p>Quest Points: ${questPoints.toString()}</p>             <p>Minting Points: ${mintingPoints.toString()}</p>             <p>Bonus Points: ${bonusPoints.toString()}</p>        ;
